...
class ColorController extends Controller
{
    ...
    public function index()
    {
        ...
        return response()->json(Color::where('user_id', 0)->orWhere('user_id', $id)->get());
    }
     ...
     */
    public function fetchUserColors()
    {
        $userId = auth()->id(); // Lấy ID của người dùng hiện tại
        $userCarBrands = Color::where('user_id', $userId)->get();
    ...
    public function store(Request $request)
    {
        ...
        foreach ($names as $name) {
            ...
            $cars= ColorRepository::create($name);
            $results[] = $cars;
        }
