...
class CarBrandController extends Controller
{
    ...
    public function index()
    {
        ...
        return response()->json(CarBrand::where('user_id', 0)->orWhere('user_id', $id)->get());
    }
     ...
     */
    public function fetchUserCarBrands()
    {
        $userId = auth()->id(); // Lấy ID của người dùng hiện tại
        $userCarBrands = CarBrand::where('user_id', $userId)->get();
    ...
    public function store(Request $request)
    {
        ...
        foreach ($names as $name) {
            ...
            $carBrand = CarBrandRepository::create($name);
            $results[] = $carBrand;
        }
