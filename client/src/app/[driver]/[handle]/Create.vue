...
<script lang="ts" setup>
  ...
  import {
    SettingResType,
    SettingCreateResType,
  } from '@/schemaValidations/settings.schema';
  ...
  const newProgram = ref<DriverCreateResType>({
    images: [],
    quantity: 5,
  });
  ...
  const errors = ref<DriverCreateResType>({});
  ...
  const onSubmit = async () => {
    try {
      ...
      newProgram.value.images.forEach((file, index) => {
        formData.append('img[]', file as Blob); // Gửi tệp dưới tên trường 'img[]'
        const test = file as Blob;
        console.log(test);
      });

      const result = await driverRequestApi.create(
        formData as DriverCreateResType
      );
  ...
  const resetProgram = (phone: string, address: string) => {
    reset();
    images.value = [];
    newProgram.value = {
      images: [],
  ...
  const me = async () => {
    ...
    if (res.payload.data.details) {
      const {phone, address} = res.payload.data.details;
      if (phone && address) {
        resetProgram(
          res.payload.data.details.phone,
          res.payload.data.details.address
        );
      }
    }
  };
